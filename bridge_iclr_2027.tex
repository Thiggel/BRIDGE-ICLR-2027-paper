...
We pretrain on five unlabeled source regimes, the long-tailed ImageNet-100-LT~\citep{imagenet-lt}, CIFAR-10-LT and CIFAR-100-LT~\citep{krizhevsky2009learning}, and 10{,}000-image subsets of the web corpora PASS~\citep{pass} and DiffusionDB~\citep{diffusiondb}. Transfer is measured on seven benchmarks, CIFAR-10-LT, CIFAR-100-LT, Stanford Cars~\citep{cars}, FGVC Aircraft~\citep{aircraft}, Oxford Flowers~\citep{flowers}, Oxford-IIIT Pets~\citep{pets} and ImageNet-100-LT, by linear probe and by $k$NN classification on frozen features. Every number is a three-seed mean and standard deviation, paired comparisons share seeds and checkpoints, and ResNet-50 is the default backbone. The baselines train five cycles of 100 epochs on a source that does not grow, while \method\ trains the same epochs on a source that grows from about 10{,}000 to about 20{,}000 images, so it receives about 1.5 times the gradient updates in Tables~\ref{tab:main_transfer_linear} and~\ref{tab:main_transfer_knn}, while the cycle ablation and the paired ViT-S experiments match updates exactly. The seed standard deviation of a seven-task average is 0.6 to 1.5 points, so differences below about one point are inside seed variation. Appendix~\ref{app:implementation} gives implementation details.
...
ImageNet is class-balanced by construction, so the first question is what a class imbalance costs once it is imposed on purpose. Two sources are drawn from the same pool, one with the power-law profile of Section~\ref{sec:setup} and one subsampled uniformly at its average retention rate, so they differ only in how images are spread over classes. The two arms are close to indistinguishable, 31.1 against 31.2 on the linear-probe seven-task average and 26.2 against 26.3 under $k$NN. \method\ on the imbalanced source reaches 42.3 and 33.6, which is 11.1 and 7.3 points above that arm and a similar margin above the balanced one. The class distribution of the source therefore accounts for little of the downstream gap, and adding support where the encoder is sparse accounts for much of it. The remaining experiments use sources whose imbalance is not imposed by us.

\subsection{Transfer across five source regimes}
\input{tables/main_transfer_all_linear}
\input{tables/main_transfer_all_knn}
\method\ has the best seven-task average in every source regime under both protocols (Tables~\ref{tab:main_transfer_linear} and~\ref{tab:main_transfer_knn}), and against SimCLR and TS it is best in 31 of 35 linear-probe cells and 33 of 35 $k$NN cells. The gain over source-matched SimCLR is above ten linear-probe points under CIFAR-100-LT and above five under CIFAR-10-LT, the two smallest and most skewed sources, and 2.4 to 3.1 points under the three larger sources. TS wins the in-domain target under the ImageNet-100-LT and PASS sources, and SDCLR's pruning schedule was tuned for larger sources, so at 10{,}000 images its averages fall to 13 to 15. The DiffusionDB source is itself synthetic and carries a generator's stylistic bias, so the gain there does not come from added natural-image diversity.
...
\end{minipage}
\end{figure}
Under the frozen cycle-0 encoder (Figure~\ref{fig:fixed-support}), inserting the generated variants reduces the radius of the treated anchors by 4.7 percent against 2.4 percent for controls matched within class on initial radius, a paired difference of $-2.3$ points. Purity falls by 0.6 points at the anchors and by 2.0 at the controls, so the variants enter the anchors' neighborhoods as same-class support and reach the controls as dilution. The diagnostic index uses $k=20$, so this radius differs from the $k=100$ selection score. Per-seed values are in Appendix~\ref{app:geometry}.
...
\section{Conclusion}
\method\ repairs a pretraining set without labels. It reads the encoder's own neighborhood geometry, selects a sparse band of images away from the extreme tail, generates local variants with a frozen diffusion model and retrains, over five cycles. The analysis of Section~\ref{sec:theory} puts the budget in that band under diminishing returns to local support and fidelity that decays in the extreme tail. Transfer improves on five pretraining sources and under four self-supervised objectives, the loop reaches the accuracy of an oracle that restores withheld real images, the acquisition rule separates from uniform and top-tail acquisition by eight points, and a frozen-encoder test places the generated images in their anchors' neighborhoods.

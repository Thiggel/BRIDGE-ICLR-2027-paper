...
We pretrain on five unlabeled source regimes, the long-tailed ImageNet-100-LT~\citep{imagenet-lt}, CIFAR-10-LT and CIFAR-100-LT~\citep{krizhevsky2009learning}, and 10{,}000-image subsets of the web corpora PASS~\citep{pass} and DiffusionDB~\citep{diffusiondb}. Transfer is measured on seven benchmarks, CIFAR-10-LT, CIFAR-100-LT, Stanford Cars~\citep{cars}, FGVC Aircraft~\citep{aircraft}, Oxford Flowers~\citep{flowers}, Oxford-IIIT Pets~\citep{pets} and ImageNet-100-LT, by linear probe and by $k$NN classification on frozen features. Every number is a three-seed mean and standard deviation, paired comparisons share seeds and checkpoints, and ResNet-50 is the default backbone for every method. The baselines train five cycles of 100 epochs on a source that does not grow, while \method\ trains the same epochs with $k=100$, $B=500$ and $G=5$ on a source that grows from about 10{,}000 to about 20{,}000 images, so it receives about 1.5 times the gradient updates of the baselines in Tables~\ref{tab:main_transfer_linear} and~\ref{tab:main_transfer_knn}, while the cycle ablation and the paired ViT-S experiments match updates exactly. The seed standard deviation of a seven-task average is 0.6 to 1.5 points, so differences below about one point are inside seed variation. DINO and MAE follow their published ViT-S recipes, and Appendix~\ref{app:implementation} gives implementation details.

\paragraph{Long-tailed construction.}
ImageNet-100 is class-balanced, so we construct its long-tailed version. A Pareto profile with exponent $\alpha=2.5$ over the 100 class ranks is rescaled to per-class retention counts between 1{,}280 and 5 images and permuted over the classes, and each image is kept with the retention probability of its class. The balanced control of Section~\ref{sec:controlled} uses the average of those probabilities for every class, so the two sources have the same expected size and a different class distribution. CIFAR-10-LT and CIFAR-100-LT use their standard configurations.

\paragraph{Compared methods and controls.}
The source-regime comparison covers source-matched SimCLR, TS, SDCLR and \method, with hybrids in the appendix. All ablations use ImageNet-100-LT source pretraining, the same downstream suite and three seeds under the full five-cycle protocol. The generator ablation includes an oracle that restores 2{,}500 withheld real images per repair, the same count as the generated variants, placing true support at the true deficit that no label-free method can see.

\section{Results}
\label{sec:results}
\subsection{Controlled imbalance on ImageNet-100}
\label{sec:controlled}
\input{tables/main_controlled_imbalance}
ImageNet is class-balanced by construction, so the first question is what a class imbalance costs once it is imposed on purpose. Two sources are drawn from the same pool, one with the power-law profile of Section~\ref{sec:setup} and one subsampled uniformly at the average retention rate of that profile, so they differ only in how the images are spread over classes. The two arms are close to indistinguishable, 31.1 against 31.2 on the linear-probe seven-task average and 26.2 against 26.3 under $k$NN. \method\ on the imbalanced source reaches 42.3 and 33.6, which is 11.1 and 7.3 points above that arm and a similar margin above the balanced one. The class distribution of the source therefore accounts for little of the downstream gap, while adding support where the encoder is sparse accounts for a large part of it. The remaining experiments ask what the repair is worth on sources whose imbalance is not imposed by us.

\subsection{Transfer across five source regimes}
\input{tables/main_transfer_all_linear}
\input{tables/main_transfer_all_knn}
\method\ has the best seven-task average in every source regime under both protocols (Tables~\ref{tab:main_transfer_linear} and~\ref{tab:main_transfer_knn}), and against SimCLR and TS it is best in 31 of 35 linear-probe cells and 33 of 35 $k$NN cells. The gain over source-matched SimCLR is above ten linear-probe points under CIFAR-100-LT and above five under CIFAR-10-LT, the two smallest and most skewed sources, and between 2.4 and 3.1 points under the three larger sources. On the four fine-grained tasks that are never a source the mean gain over SimCLR is 1.7 to 7.6 linear-probe points. TS wins the in-domain target under the ImageNet-100-LT and PASS sources and stays below SimCLR on Cars and Aircraft everywhere, and SDCLR's pruning schedule was tuned for larger sources, so at 10{,}000 images its averages fall to 13 to 15. The DiffusionDB source is itself synthetic and carries a generator's stylistic bias, so the gain there does not come from added natural-image diversity.

\subsection{Pre-training objectives and end-to-end adaptation}
\label{sec:objectives}
\input{tables/main_vits_objectives}
Four objectives run paired on ViT-S under their reference recipes and one repair budget, and \method\ raises the seven-task linear-probe average of each, by 2.0 points for SimCLR, 2.8 for MoCo v3 and 2.2 for DINO, and by 0.8 for MAE, which is inside seed variation. Under $k$NN the MoCo v3 average is unchanged to slightly lower, 33.0 against 32.8, while the MAE average rises 2.0 points. Of the 14 linear-probe and $k$NN measurements per objective, 12 or 13 improve, and the fine-grained targets gain most, up to 7.1 points on Pets for DINO. The same ResNet-50 representations improve after end-to-end adaptation, by 2.0 and 3.2 points of fine-tuning accuracy with 1 and 10 percent of the labels and by 2.4 mIoU on PASCAL VOC segmentation.
...
\paragraph{Generator.} Generated repair with Stable Diffusion 3 matches the oracle of Section~\ref{sec:setup}, 41.5 against 40.7. FLUX at six sampling steps and guidance 2.5 reaches 35.0.

\paragraph{Selection.} The three mode windows with lower cutoffs at the 1st, 50th and 75th percentile reach 42.1 to 42.3 and are not separated, while uniform acquisition reaches $35.5\pm1.2$ and the top tail $34.3\pm1.2$. Welch t-tests with Holm adjustment place every window ahead of both at adjusted $p\leq.011$, which supports the upper trim of Proposition~\ref{prop:interior} and leaves its lower boundary unresolved. The arms vary trimming and farthest-point diversification together, so the 6.6 to 8.0 points cover both.

\paragraph{Cycles and backbone.} At fixed total repair volume and optimizer budget, five cycles improve the seven-task average over two by 7.3 points and remain 8.2 points ahead of ten. Two cycles place most of the budget before the representation has changed and ten leave too little training between repairs. ResNet-50 is the strongest backbone at this source size.
...
\caption{Fixed-encoder intervention test. With the cycle-0 encoder frozen and every generated image in the $k$NN index, the radius of the treated anchors falls about twice as far as that of matched controls, so the variants land in their own anchors' neighborhoods, and purity falls about a third as much at the anchors. Bars are seed means with one standard deviation.}
\label{fig:fixed-support}
\end{minipage}\hfill
\begin{minipage}[t]{0.50\linewidth}
\centering
\includegraphics[width=0.8\linewidth]{figures/cycle_score_histograms.pdf}
\caption{Score distribution over the four repair cycles of one ImageNet-100-LT run, with the 75th to 99th percentile band shaded and its modal bin marked. The distribution shifts to larger scores at every cycle, so the band is re-derived from the current representation.}
\label{fig:cycle-histograms-main}
\end{minipage}
\end{figure}
Under the frozen cycle-0 encoder (Figure~\ref{fig:fixed-support}) we ask whether the generated variants land in the neighborhoods of their own anchors. Inserting them reduces the radius of the treated anchors by 4.7 percent against 2.4 percent for controls matched within class on initial radius, a paired difference of $-2.3$ points. Purity falls by 0.6 points at the anchors and by 2.0 at the controls, so the variants enter the anchors' neighborhoods as same-class support and reach the controls as dilution. The diagnostic index uses $k=20$, so this radius differs from the $k=100$ selection score. All 500 cycle-0 anchors have selection scores inside the band (Figure~\ref{fig:mode-window}), with per-seed values in Appendix~\ref{app:geometry}.

\FloatBarrier
\vspace{-14pt}
\section{Limitations}
\method\ depends on informative encoder neighborhoods in the first cycle and on faithful local generation under a frozen generator that imports the biases of its own pretraining data. Repair fidelity, the $\rho$ of Section~\ref{sec:theory}, is not measured in pixel or semantic space, and the deficit of the top-tail arm is consistent with low fidelity there. Later cycles select generated images as anchors, 7 to 13 percent at cycle 1 and 34 to 45 percent at cycle 4, with no filter applied. The band, neighborhood size and budget were held at their defaults across all five sources. The loop raises wall-clock time by roughly 30 to 45 percent, almost all of it diffusion sampling without caching (Appendix~\ref{app:compute}), which a distilled few-step sampler would largely remove.

\vspace{-14pt}
\section{Conclusion}
\method\ repairs a pretraining set without labels. It reads the encoder's own neighborhood geometry, selects a sparse band of images away from the extreme tail, generates local variants with a frozen diffusion model and retrains, over five cycles. The analysis of Section~\ref{sec:theory} puts the budget in that band under diminishing returns to local support and fidelity that decays in the extreme tail, and makes reallocation after every repair optimal. Transfer improves on five pretraining sources and under four self-supervised objectives, the loop reaches the accuracy of an oracle that restores withheld real images, the acquisition rule separates from uniform and top-tail acquisition by eight points, and a frozen-encoder test places the generated images in the neighborhoods of the selected images.

The loop does not yet measure the fidelity of its own repairs, so the generator's biases stay unaccounted for. An estimate of fidelity would close that gap and give the band a criterion of its own in place of fixed percentiles. The same reading of the geometry applies wherever the data are fixed and the objective is not, which leaves other modalities to try.

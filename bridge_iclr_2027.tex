...
We compare with two groups of methods. TS and SDCLR change the loss on a fixed long-tailed source and are trained on each of the five sources, with hybrids in the appendix. TADA, AIDE and DiffAug add generated images as \method\ does, so they start from the same ImageNet-100-LT checkpoint per seed and follow the same five cycles, and only the choice or generation of images differs. All ablations use ImageNet-100-LT pretraining, the same downstream suite and three seeds under the five-cycle protocol. The generator ablation includes an oracle that restores 2{,}500 withheld real images per repair, the same count as the generated variants, placing true support at a deficit no label-free method can see.
...
\paragraph{Prior methods.} Table~\ref{tab:main_prior} compares \method\ with three methods that also add generated images. \method\ has the highest average under both probes, 2.8 linear-probe points and 1.5 $k$NN points above the best prior method, and the highest accuracy on six of the seven targets. The exception is Flowers, where TADA reaches 39.2 against 36.8. No prior method raises either average by more than 0.3 points over SimCLR. TADA generates variants of the examples a supervised classifier learns slowly. Without labels, we select the 500 images with the highest InfoNCE loss under the current encoder and repair them with the image-to-image step of \method. AIDE samples images inversely to the size of their cluster in CLIP ViT-B/32 feature space, captions them with BLIP and generates new images from the caption alone. Both use the same generator and image budget as \method, so the gap comes from which images are chosen and how they are generated. DiffAug trains its own diffusion generator jointly with the encoder and reaches 11.3, and adding \method\ repair to it changes the average by less than half a point.
...
\paragraph{Selection.} The three mode windows with lower cutoffs at the 1st, 50th and 75th percentile reach 42.1 to 42.3 and are not separated, while uniform acquisition reaches $35.5\pm1.2$ and the top tail $34.3\pm1.2$ (Welch t-tests with Holm adjustment, $p\leq.011$ for every window against both). This agrees with Proposition~\ref{prop:interior}, where excluding the extreme tail raises the objective, and leaves the lower boundary of the band unresolved. Each window changes trimming and diversification at once, and the 6.6 to 8.0 points include both effects. The inverse-occupancy rule of AIDE on $\sqrt{N}$ spherical $k$-means clusters of the encoder features reaches 37.9.

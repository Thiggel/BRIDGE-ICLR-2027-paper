...
\caption{Fixed-encoder intervention test. With the cycle-0 encoder frozen and every generated image inserted into the $k$NN index, the radius of the treated anchors falls about twice as far as that of matched controls, so the variants land in their own anchors' neighborhoods. Purity falls about a third as much at the anchors. Bars are the mean over seeds and the error bars are one standard deviation.}
\label{fig:fixed-support}
\end{minipage}\hfill
\begin{minipage}[t]{0.53\linewidth}
\centering
\includegraphics[width=0.8\linewidth]{figures/cycle_score_histograms.pdf}
\caption{Score distribution over four cycles of one ImageNet-100-LT run with the repair loop running, above, and of a run from the same checkpoint on the same schedule with the loop switched off, below. The 75th to 99th percentile band is shaded and its modal bin is marked. The band moves to larger scores in both rows, so it is re-derived from the current representation at every cycle, and it moves slightly less when the loop is repairing.}
\label{fig:cycle-histograms-main}
\end{minipage}
\end{figure}
Under the frozen cycle-0 encoder (Figure~\ref{fig:fixed-support}) we ask whether the generated variants land in the neighborhoods of their own anchors. Inserting the generated images reduces the radius of the treated anchors by 4.7 percent against 2.4 percent for controls matched within class on initial radius, a paired difference of $-2.3$ points on average. A variant that missed its anchor's neighborhood would leave the anchor's radius unchanged, so the test measures locality together with spillover onto untreated images. Purity falls by 0.6 points at the anchors and by 2.0 at the controls, so the variants enter the anchors' neighborhoods as same-class support and reach the controls as dilution, with Proposition~\ref{prop:local-support} accounting for the sign of the change alone. The diagnostic index uses $k=20$, so this radius differs from the $k=100$ selection score. All 500 cycle-0 anchors have selection scores inside the band (Figure~\ref{fig:mode-window}), Figure~\ref{fig:cycle-histograms-main} shows why the band is re-derived every cycle, and Appendix~\ref{app:geometry} gives the per-seed values (Table~\ref{tab:fixed-support}).
...
\vspace{-6pt}
\section{Conclusion}
\method\ repairs a pretraining set without labels. It reads the encoder's own neighborhood geometry, selects a sparse band of images away from the extreme tail, generates local variants of them with a frozen diffusion model and retrains, and it repeats that over five cycles. The analysis of Section~\ref{sec:theory} puts the budget in that band under diminishing returns to local support and fidelity that decays in the extreme tail, and it makes reallocation after every repair optimal for a fixed objective. The experiments follow the same account. Transfer improves on five pretraining sources and under four self-supervised objectives, the loop reaches the accuracy of an oracle that restores withheld real images, the acquisition rule separates from uniform and top-tail acquisition by eight points over five cycles, and a frozen-encoder test places the generated images in the neighborhoods of the images that were selected.

The loop does not yet measure the fidelity of its own repairs, so the generator's biases stay unaccounted for and the share of generated anchors grows with the cycle count. An estimate of fidelity would close that gap and would give the band a criterion of its own in place of a fixed pair of percentiles. The same reading of the geometry applies wherever the data are fixed and the objective is not, which leaves pretraining corpora in other modalities as the next thing to try.

...
\item An evaluation on five source regimes and seven transfer tasks under linear-probe and $k$NN protocols (Section~\ref{sec:results}), with paired ViT-S experiments across four SSL objectives, low-label fine-tuning, semantic segmentation, an oracle arm and matched controls for the closest prior methods.
\item An account of where the effect comes from (Sections~\ref{sec:one-stage} and~\ref{sec:geometry}). Three acquisition rules are not separated after one cycle of repair and separate after five, and a fixed-encoder intervention test shows the generated support landing at the selected anchors.
\end{itemize}
...
We pretrain on five unlabeled source regimes, the long-tailed ImageNet-100-LT~\citep{imagenet-lt}, CIFAR-10-LT and CIFAR-100-LT~\citep{krizhevsky2009learning}, and 10{,}000-image subsets of the web corpora PASS~\citep{pass} and DiffusionDB~\citep{diffusiondb}. Transfer is measured on seven benchmarks, CIFAR-10-LT, CIFAR-100-LT, Stanford Cars~\citep{cars}, FGVC Aircraft~\citep{aircraft}, Oxford Flowers~\citep{flowers}, Oxford-IIIT Pets~\citep{pets} and ImageNet-100-LT, by linear probe and by $k$NN classification on frozen features. Every number is a three-seed mean and standard deviation, and paired comparisons share seeds and checkpoints. ResNet-50 is the default backbone, common to the baselines. The baselines train five cycles of 100 epochs on a source that does not grow, while \method\ trains the same epochs with $k=100$, $B=500$ and $G=5$ on a source that grows from about 10{,}000 to about 20{,}000 images, so it receives about 1.5 times the gradient updates of the baseline in Tables~\ref{tab:main_transfer_linear} and~\ref{tab:main_transfer_knn}. The cycle ablation, the paired ViT-S experiments and the single-stage controls match updates exactly, and the conventional-augmentation control adds the same number of images. The seed standard deviation of a seven-task average is 0.6 to 1.5 points under five cycles, so differences below about one point are inside seed variation. DINO and MAE follow their published ViT recipes on ViT-S. Appendix~\ref{app:implementation} gives implementation details.
...
\paragraph{Generator.} Generated repair with Stable Diffusion 3 matches the oracle of Section~\ref{sec:setup} within three-seed variation, 41.5 against 40.7. FLUX at its original sampling settings reaches 35.0. Under matched sampling the single-stage four-task difference between the two generators narrows to within seed variation (Appendix Table~\ref{tab:generator-sensitivity}), and the five-cycle gap is not decomposed further.
...
\paragraph{Sensitivity of the acquisition rule.} Appendix Table~\ref{tab:selector-robustness-full-linear} varies the candidate-pool multiplier, upper cutoff, neighborhood size, distance metric and selection strategy one at a time. Cosine and normalized $L_2$ scoring select the identical anchor set and differ by 0.4 points on the average, which bounds run-to-run spread, and the table spans 1.3 points with no variant separated from the default at three seeds. That table uses the single-stage protocol, so it bounds the sensitivity of the acquisition rule alone.

\subsection{Single-stage against five-cycle acquisition}
\label{sec:one-stage}
\begin{figure}[t]
\centering
\includegraphics[width=0.52\linewidth]{figures/one_stage_vs_five.pdf}
\vspace{-4pt}
\caption{Left, nine bands of the score distribution of unequal percentile widths each receive the whole budget after a single stage, and their averages span 1.3 points (Appendix Table~\ref{tab:percentile-utility-full-linear}). The shaded region is one seed standard deviation of a single band's seven-task mean, 0.66 points on average and between 0.35 and 0.93, so no pair of bands is separated at three seeds. Right, three acquisition rules under five cycles separate by 8.0 points, re-plotting the selection block of Table~\ref{tab:main_ablation_combined}. Single-stage averages lie near 28 and five-cycle averages between 34 and 42.}
\label{fig:one-stage}
\vspace{-4pt}
\end{figure}
Figure~\ref{fig:one-stage} spends one identical budget on each of nine score bands, and after a single stage the densest quartile and the extreme tail are not separated at three seeds, while over five cycles the top tail, uniform acquisition and the mode window separate by 8.0 points. The protocols also differ in repair volume and training, 2{,}500 images against 10{,}000, so the comparison does not isolate re-estimation as the cause. Under the single-stage protocol the mode window is not the leader, reaching 18.9 against 20.0 for the TADA-style rule and 19.5 for the top tail on the four-task average, whose per-arm seed standard deviation is 0.1 to 1.0, and no pair of rules separates at three seeds (Appendix Table~\ref{tab:closest-prior-empirical}). A targeted rule appears to gain value because each repair changes the representation from which the next anchors are chosen, while the single-stage arms decide once from a fixed model. The DiffAug adaptation learns its generator jointly with the encoder from the 10{,}000-image source. It reaches 11.5 and falls further with mode-window anchors, below every arm that adds no generated image, since a generator learned from a small skewed source is weak.
...
\input{tables/single_stage_pipelines}

\begin{table}[h]
\centering
\small
\caption{Generator sensitivity on the four-task diagnostic (Cars, Aircraft, Flowers, ImageNet-100-LT) after one repair stage from a common checkpoint, three paired seeds. The original FLUX setting of six steps and guidance 2.5 uses the same generator setting as the 35.0 FLUX run of Table~\ref{tab:main_ablation_combined}. Guidance under an empty prompt gives bit-identical images at 3.0, 5.0 and 7.5.}
...
Scoring uses the mean squared normalized $\ell_2$ distance of Equation~\ref{eq:score}, and raw $\ell_2$, cosine and multiscale-$k$ variants are reported as a sensitivity study in Table~\ref{tab:selector-robustness-full-linear}. For unit vectors the squared distance is an affine function of the cosine, which is why the cosine row selects the identical anchor set. The mode-window selector restricts the score distribution to the 75th to 99th percentile range, builds a histogram with the automatic bin heuristic, forms a mode-centered candidate pool of size $4B$ and applies greedy farthest-point sampling on normalized features to select the final $B$ anchors.

\paragraph{Generators.}
The main method uses Stable Diffusion 3 Medium in image-to-image mode, where an empty prompt makes the guidance scale inert (Table~\ref{tab:generator-sensitivity}). The generation ablation replaces it with FLUX or with the oracle, which adds the withheld real images and runs no generator. In the PASS and DiffusionDB experiments the SD3 batch size is 12 to fit the 24-hour wall-time limit.

\paragraph{Single-stage controls.} Every arm of Table~\ref{tab:closest-prior-empirical} branches from one SimCLR ResNet-50 checkpoint per seed, adds 2{,}500 images where it generates, and trains one stage of at most 100 epochs capped at 4{,}850 optimizer steps at batch 128. The TADA-style arm ranks source images by the decrease of their SSL loss over the first stage and selects the slowest-learned 500. The inverse cluster-frequency arm clusters the current embeddings and samples anchors with probability inverse to cluster occupancy. The conventional-augmentation arm applies the standard SimCLR augmentation pipeline five times to each mode-window anchor in place of the generator. The DiffAug arm trains a conditional diffusion generator jointly with the encoder on the source, following the published objective, and its \method\ variant conditions that generator on the mode-window anchors. The distillation arm adds a feature-matching loss between the encoder and frozen SD3 activations on the source images and adds no image.

...
\section{Conclusion}
Without labels, \method\ selects a sparse band of images by neighbor distance, adds local variants from a frozen diffusion model and retrains, over five cycles. The band follows from diminishing returns to local support and fidelity decay in the extreme tail, beats uniform and top-tail acquisition by up to eight points, and raises transfer on five sources and four objectives by up to 10.6 linear-probe points, matching an oracle that adds withheld real images.

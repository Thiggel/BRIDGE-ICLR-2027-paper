...
\section{Conclusion}
In this paper we introduce \method, a framework in which a self-supervised learner repairs the sparse regions of its own representation. It selects a band of sparse images by neighbor distance, adds local variants from a frozen diffusion model and retrains, without labels and without changing the SSL objective. Our analysis shows that the budget belongs in a sparse band that excludes the extreme tail, and the experiments agree: the band beats uniform and top-tail acquisition, transfer improves on five sources and four objectives, and generated repair matches an oracle that adds withheld real images.

Since \method\ needs only an encoder and a generator, it can be added to existing SSL pipelines. Web-scale pretraining corpora are long-tailed in the same way as our sources, and as generators improve, the representation itself could decide at each stage of large-scale pretraining which regions to fill, making data curation part of training. The same loop applies to any modality with a pretrained generator, such as audio, video or molecules.
